...
class ArticulationParameter {
 ...
 public:
  ...
  void SetChangeIndicator(uint8_t value) { change_indicator_ = value; }

  [[nodiscard]] auto GetPartAttachedTo() const -> uint16_t {
    return part_attached_to_;
  }

  void SetPartAttachedTo(uint16_t value) { part_attached_to_ = value; }
  ...
  void SetParameterValue(double value) { parameter_value_ = value; }
  ...
  [[nodiscard]] auto GetMarshalledSize() const -> std::size_t {
    return sizeof(parameter_type_designator_) + sizeof(change_indicator_) +
           sizeof(part_attached_to_) + sizeof(parameter_type_) +
           sizeof(parameter_value_);
  }
};
